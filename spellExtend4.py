...
def do(filename,abuseFile):
    frequencies = open("englishFrequenciesNew2.txt","a")
...
def reformed_do(filename,abuseFile):
    ...
    frequencies = open("englishFrequenciesNew_5.txt","a")
    noAbuse=open("noSwearNew_5.txt","a")
    for tweet in tweets:
        row = tweet
        #print tweet
                ...
                for word in tweet.split(' '):
                    ...
                    if(found_a_w==0):
                        if(count == 0):# if zero masks   ### Why only for zero masks
                            ...
                            if(temp_abuse==temp_word and temp_word!= "as" and temp_word!= "but"):
                                frequencies.write(abuse+'\t'+word+'\t'+filename+'\t'+row)
                                found = found +1
                                found_a = found_a +1
                                found_a_w = found_a_w +1
                                print abuse +" "+word+" "+temp_word+" removed repeated"

                    if(False):
                        value = 0#reformed_edit_dist(word,abuse)
...
abuseFiles = []
tweetFiles =[]
path = "/home/du3/13CS30042/sc_jeenu/socialComputing"
getFiles("abuseFolder/englishSwearsNew2.txt",abuseFiles,path)
